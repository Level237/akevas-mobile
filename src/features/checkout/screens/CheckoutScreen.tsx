...
const CheckoutScreen = ({ params }: Props) => {
  ...
  const { data: quartersData, isLoading: quartersLoading } =
    useGetQuartersQuery(undefined);
  ...
  const variationInfo = params.variationParams
    ? JSON.parse(params.variationParams)
    : null;
  ...
  const uniqueCities = useMemo(
    () =>
      Array.from(new Set(cartItems.map((item: any) => item.product.residence))),
    [cartItems],
  );
  ...
  const filteredQuarters = useMemo(() => {
    ...
    const baseQuarters = quartersData.data.filter((q: any) => {
      ...
      return (
        q.town_name ===
        (isMultiCity ? selectedCity || uniqueCities[0] : productLocation)
      );
    });
  ...
  const handlePayment = useCallback(() => {
    if (
      (deliveryOption === "localDelivery" ||
        deliveryOption === "remoteDelivery") &&
      !selectedQuarter
    ) {
    ...
    if (!paymentPhone || paymentPhone.length < 9) {
      Alert.alert(
        "Erreur",
        "Veuillez entrer un numéro de téléphone de paiement valide",
      );
      return;
    }
    if (!firstName) {
    ...
    const orderData = {
      ...
      delivery_info: isMultiCity
        ? shippingFee === 1500
          ? `Récupérer en magasin de ${selectedCity}`
          : `Expédition et livraison à domicile dans la ville de ${selectedCity}`
        : null,
    };
  ...
  return (
    ...
    >
      ...
      >
          ...
          <View style={styles.warningCard}>
            <View style={styles.warningHeader}>
              ...
              <Text style={styles.warningTitle}>
                Produits dans différentes villes
              </Text>
            </View>
            <Text style={styles.warningText}>
              Les produits de votre panier ne sont pas tous situés dans la même
              ville.
            </Text>
            <TouchableOpacity
            ...
            >
              <Text style={styles.viewByCityBtnText}>
                Voir les produits par ville
              </Text>
            </TouchableOpacity>
          </View>
        )}
          ...
          renderSection(
            ...
            <View style={styles.form}>
              ...
              >
                <Text
                  style={
                    selectedCity ? styles.inputText : styles.placeholderText
                  }
                >
            ...
            {s === "1" ? (
                        ...
                        <View style={styles.row}>
                          <View
                            style={[
                              ...
                              {
                                backgroundColor:
                                  item.selectedVariation.color?.hex || "#CCC",
                              },
                              { marginRight: 4 },
                            ]}
                          />
              ...
              <View style={styles.productListItem}>
                ...
                <View style={styles.productItemDetails}>
                  ...
                  <Text style={styles.productItemPrice}>
                    {unitPrice.toLocaleString()} FCFA x {quantity}
                  </Text>
                  <View style={[styles.row, { marginTop: 4, alignItems: "center" }]}>
                    <Ionicons name="location-outline" size={14} color="#6B7280" />
                    <Text style={{ fontSize: 12, color: "#6B7280", marginLeft: 2 }}>
        ...
        {(!isMultiCity || (isMultiCity && selectedCity)) &&
          renderSection(
            "Options de livraison",
            <Truck size={20} color={COLORS.primary} />,
            <View style={styles.deliveryOptionsGrid}>
              {(!isMultiCity
                ? ["pickup", "localDelivery", "remotePickup", "remoteDelivery"]
                : (["pickup", "remoteDelivery"] as DeliveryOption[])
              ).map((option) => (
                <TouchableOpacity
                  key={option}
                  style={[
                ...
                >
                  <Text
                    style={[
                      styles.optionLabel,
                      deliveryOption === option && styles.activeOptionLabel,
                    ]}
                  >
                    {option === "pickup" &&
                      (isMultiCity
                  ...
                  <Text style={styles.optionFee}>
                    {option === "pickup" && isMultiCity
                      ? "+1,500 FCFA"
                      : DELIVERY_FEES[option as DeliveryOption] === 0
                        ? "Gratuit"
                        : `+${DELIVERY_FEES[option as DeliveryOption].toLocaleString()} FCFA`}
                  </Text>
          ...
          renderSection(
            ...
            <View style={styles.form}>
              ...
              >
                <Text
                  style={
                    selectedQuarter ? styles.inputText : styles.placeholderText
                  }
                >
          ...
          <View style={styles.paymentContainer}>
            <View style={styles.paymentMethods}>
              <TouchableOpacity
                style={[
                  styles.paymentBtn,
                  selectedPayment === "cm.orange" && styles.activePayment,
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.paymentBtn,
                  selectedPayment === "cm.mtn" && styles.activePayment,
                ]}
        ...
        <View style={styles.costSummary}>
          ...
          <View style={styles.costRow}>
            ...
            <Text style={styles.costValue}>
              {Math.round(serviceFee).toLocaleString()} FCFA
            </Text>
          </View>
        ...
        >
          {isProcessing ? (
            ...
            <Text style={styles.payButtonText}>
              Payer {Math.round(total).toLocaleString()} FCFA
            </Text>
          )}
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.cityModalContent}>
            ...
            <ScrollView style={styles.cityList}>
              {uniqueCities.map((city) => {
                const cityProducts = cartItems.filter(
                  (item: any) => item.product.residence === city,
                );
                return (
                  <View key={city} style={styles.citySection}>
                    <TouchableOpacity
                      style={[
                        styles.cityHeader,
                        selectedCity === city && styles.activeCityHeader,
                      ]}
                      onPress={() => {
                      ...
                      {cityProducts.map((item: any, idx: number) => {
                        ...
                        return (
                          <View key={idx} style={styles.cityProductItem}>
                            <Image
                              source={{
                                uri: normalizedProduct.product_profile,
                              }}
                              style={styles.tinyImage}
                            />
                            <View>
                              <Text style={styles.tinyName}>
                                {normalizedProduct.product_name}
                              </Text>
                              <Text style={styles.tinyQty}>
                                Qté: {item.quantity}
                              </Text>
                            </View>
                          </View>
