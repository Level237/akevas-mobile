...
const ProductDetailScreen = ({ url }: Props) => {
    ...
    if (selectedVariant?.attributes && selectedVariant.attributes.length > 0) {
      return (
        selectedVariant.attributes.find(
          (attr: any) => attr.value === selectedAttribute?.value,
    ...
    if (product?.variations && product.variations.length > 0) {
      ...
      if (currentVariant.attributes && currentVariant.attributes.length > 0) {
        ...
        if (product.productWholeSales && product.productWholeSales.length > 0) {
          ...
          const sortedWholesale = Array.from(product.productWholeSales).sort(
            (a: any, b: any) => Number(b.min_quantity) - Number(a.min_quantity),
          );
          ...
          selectedAttr.wholesale_prices &&
          selectedAttr.wholesale_prices.length > 0
        ) {
          // Prix de gros au niveau attribut (pour produits variés couleur + attribut)
          const sortedAttrWholesale = Array.from(
            selectedAttr.wholesale_prices,
          ).sort(
            (a: any, b: any) => Number(b.min_quantity) - Number(a.min_quantity),
          );
        ...
        return {
          ...
          images:
            currentVariant.images?.map((path: string) => ({ path })) || [],
  ...
  const handleVariantSelect = useMemo(
    () => (variant: any) => {
  ...
  const getMinWholesaleQty = useCallback(() => {
    ...
    return source
      ...
      .reduce(
        (min: number, q: number) => (min === 0 ? q : Math.min(min, q)),
        0,
      );
  }, [getWholesaleSource]);
