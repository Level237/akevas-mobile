...
const WishlistScreen = () => {
  ...
  } = useGetHomeProductsQuery(undefined, {
  ...
  const rawRecommendations =
    activeData?.data || activeData?.productList || activeData || [];
  const recommendations = Array.isArray(rawRecommendations)
    ? rawRecommendations.map(normalizeProduct)
    : [];
  ...
  const handleRemove = (item: any) => {
    ...
    dispatch(
      removeFavorite({
        ...
        selectedVariation: variationToRemove,
      }),
    );
    Toast.show({
      type: "info",
      ...
      <View style={styles.recommendationBox}>
        ...
        {isRecommendationsLoading ? (
          ...
          <FlatList
            ...
            contentContainerStyle={styles.recommendationList}
            renderItem={({ item }) => (
              <RecommendationItem item={item} onPress={handleProductPress} />
            )}
          />
        )}
      </View>
